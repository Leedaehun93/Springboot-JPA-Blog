...
public class User {
     ...
     * 12345 => (비밀번호는 해시 암호화를 고려하여 길이를 100으로 설정)
     */
    ...
    /**
     * 권한 부여 필드 : 컬럼에 기본 값(사용자 역할(role).
     * 기본 값은 ('user')을 지정해주고 문자열 값에는 반드시 홑 따옴표('') 사용하여 문자라고 알려줘야 한다.
    */
    //    @ColumnDefault("'user'")
    /**
     * private RoleType role;  => enum 열거형을 생성하여 role 필드에 허용된 값들을 USER 와 ADMIN으로 제한합니다.
     ...
     *
     * @Enumerated(EnumType.STRING) : 데이터베이스에 enum 값을 저장할 때, 어노테이션을 사용하여 enum 값이 문자열로 저장되도록 지정한다.
     */
     ...
     *
     * @UpdateTimestamp 어노테이션을 사용하여 업데이트 시간 필드를 활용 가능 예: private Timestamp updateDate;
